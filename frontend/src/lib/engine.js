...
export function opensslCommand(opts) {
  ...
  // Ed25519 and PQC schemes ignore the message-digest flag (fixed scheme)
  const hashFlag = (opts.keyType === "ed25519" || opts.keyType === "pqc")
    ? "" : "-" + (opts.hash || "SHA-256").toLowerCase().replace("-", "");
